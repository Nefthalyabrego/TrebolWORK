...
jQuery(document).ready(function($){
    ...
    $( 'input[name=rara-ecommerce-flush-local-fonts-button]' ).on( 'click', function( e ) {
        var data = {
            wp_customize: 'on',
            action: 'rara_ecommerce_flush_fonts_folder',
            nonce: rara_ecommerce_cdata.flushFonts
        };  
        $( 'input[name=rara-ecommerce-flush-local-fonts-button]' ).attr('disabled', 'disabled');

        $.post( ajaxurl, data, function ( response ) {
            if ( response && response.success ) {
                $( 'input[name=rara-ecommerce-flush-local-fonts-button]' ).val( 'Successfully Flushed' );
            } else {
                $( 'input[name=rara-ecommerce-flush-local-fonts-button]' ).val( 'Failed, Reload Page and Try Again' );
            }
        });
    });
