...
function rara_ecommerce_customize_script(){
    $array = array(
		...
		'home'       => get_permalink( get_option( 'page_on_front' ) ),
        'flushFonts' => wp_create_nonce( 'rara-ecommerce-local-fonts-flush' ),
 ...
 * @return void
 */
function rara_ecommerce_ajax_delete_fonts_folder() {
    // Check request.
    if ( ! check_ajax_referer( 'rara-ecommerce-local-fonts-flush', 'nonce', false ) ) {
        wp_send_json_error( 'invalid_nonce' );
    }
    if ( ! current_user_can( 'edit_theme_options' ) ) {
        wp_send_json_error( 'invalid_permissions' );
    }
    if ( class_exists( '\Rara_eCommerce_WebFont_Loader' ) ) {
        $font_loader = new \Rara_eCommerce_WebFont_Loader( '' );
...
add_action( 'wp_ajax_rara_ecommerce_flush_fonts_folder', 'rara_ecommerce_ajax_delete_fonts_folder' );
